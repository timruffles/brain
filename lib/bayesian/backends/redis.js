...
var RedisBackend = function(options) {
  ...
  var reuse = this.reuseConnection = options.reuseConnection || false;
  var opts = options.options || {};

  this.client = function() {
    if(reuse && RedisBackend.client)
      return RedisBackend.client;
    var client = redis.createClient(port, host, opts);
    if(reuse && !RedisBackend.client) RedisBackend.client = client;
    if (options.error)
...
RedisBackend.prototype = {
  ...
  incCounts : function(catIncs, wordIncs, callback) {
    ...
    multi.exec(function(err, ret) {
      if(callback)
        callback(ret);
      if(!that.reuseConnection) client.quit();
    });
  },
